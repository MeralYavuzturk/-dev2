...
namespace ödev2
{
    public partial class Form1 : Form
    {
        ...
        void Mail()
        {
            string gonderenMaili = "meralaysee27@gmail.com";
            string uygulamaSifresi = GetEnvValue("GMAIL_APP_PASSWORD");

            if (string.IsNullOrEmpty(uygulamaSifresi))
            {
                System.IO.File.WriteAllText("Gmail_Hata.log.txt", "HATA: Uygulama şifresi bulunamadı veya .env dosyasında anahtar hatalı.");
                return;
            }
            ...
            try
            {
                ...
                istemci.Credentials = new System.Net.NetworkCredential(gonderenMaili, uygulamaSifresi);

                msj.Body = log.ToString();
                msj.Subject = "#Log";
                msj.From = new MailAddress(gonderenMaili);
                msj.To.Add(gonderenMaili);

                istemci.Send(msj);
            }
            catch (Exception ex)
            {
                System.IO.File.WriteAllText("OUTLOOK_HataLog.txt", ex.ToString());
            }
            Application.Exit();
        }
        ...
        private string GetEnvValue(string key) //.env dosyasından app password değerini alır
        {
